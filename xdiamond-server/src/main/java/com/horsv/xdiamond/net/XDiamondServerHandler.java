...
import com.horsv.xdiamond.domain.Profile;
import com.horsv.xdiamond.domain.Project;
import com.horsv.xdiamond.domain.vo.ResolvedConfig;
...
public class XDiamondServerHandler extends SimpleChannelInboundHandler<Message> {
  ...
  protected void channelRead0(ChannelHandlerContext ctx, Message msg) throws Exception {
    ...
    if (msg.getType() == Message.REQUEST) {
      Message.MessageBuilder messageBuilder = Message.response();

      Request request = msg.dataToRequest();
      // response里的id设置为request的id
      Response.ResponseBuilder responseBuilder = Response.builder().id(request.getId());
